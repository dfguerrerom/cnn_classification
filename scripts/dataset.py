...
class PlanetDataSet(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if self.multilabel:
            ...
            label = torch.LongTensor(
                [
                    self.class_to_idx[lbl]
                    for lbl in self.data_df.loc[index][self.label_col].split(self.sep)
                ]
            )
    ...
    def plot_tags(self, size=20):
        ...
        for idx, tag in enumerate(tags):
            ...
            image_id = (
                self.data_df[self.data_df[self.label_col].isin(tags)]
                .sample()
                .index.values[0]
            )
    ...
    def get_tags(self):
        ...
        tags = (
            ...
            else [tag for row in df.values for tag in row.split(self.sep)]
        )
